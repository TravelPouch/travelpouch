        ...
        DropdownMenu(
            ...
            locationSuggestions.filterNotNull().take(3).forEach { location ->
                DropdownMenuItem(
                    ...
                    onClick = {
                        locationViewModel.setQuery(location.name) // Set query to the selected location name
                        setSelectedLocation(location) // Update the selected location
                        locationQuery.value = location.name // Update the input field with the selected location name
                        setShowDropdown(false) // Close dropdown after selection
                    },
                    modifier = Modifier
                        .padding(8.dp)
                        .testTag("suggestion_${location.name}")
                )
