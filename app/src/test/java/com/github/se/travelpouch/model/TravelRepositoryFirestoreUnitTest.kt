...
class TravelRepositoryFirestoreUnitTest {
    ...
    fun deletesTravelByIdSuccessfully() {
        val task: Task<Void> = mock()
        whenever(mockFirestore.collection("travels").document(travel.fsUid).delete()).thenReturn(
            task
        )
        whenever(task.isSuccessful).thenReturn(true)
        ...
        mockStatic(Log::class.java).use { logMock ->
            ...
            logMock.verify {
                Log.e(
                    ...
                    "Error converting document to TravelContainer",
                    exception
                )
            }
        }
    ...
    fun convertsDocumentToTravelWithValidLatitude() {
        ...
        whenever(document.getString("description")).thenReturn("Test Description")
        whenever(document.getTimestamp("startDate")).thenReturn(Timestamp.now())
        whenever(document.getTimestamp("endDate")).thenReturn(
            Timestamp(
        ...
        val method = travelRepositoryFirestore::class.java.getDeclaredMethod(
            "documentToTravel",
    ...
    fun getTravels_documentConversionThrowsException() {
        ...
        mockStatic(Log::class.java).use { logMock ->
            ...
            assertTrue(successCalled)
            logMock.verify { Log.e("TravelRepositoryFirestore", "Error converting document to TravelContainer", exception) }
        }
    }
